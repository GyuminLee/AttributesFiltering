...
public class AbFAlgorithm {
	...
	public static XLog doFiltering(XLog originalLog, List<String> selectedAttribute, double thresholdValue, double waitingTimeValue, double dfrValue, int filteringType) {
		...
		if(filteringType == 0 ) { // Label
			System.out.println("Lable Log filtering");
			...
			for(XTrace trace: log) {
				if(removeList.contains(log.indexOf(trace))) { // Noise
		...
		} else if(filteringType == 1) { // Only Noise
			System.out.println("Noise Log filtering");
			for(XTrace trace : log) {
				String eventList = "";
				int counter = 0;
				String prevTime = "";
			...
			XLog noiseLog = (XLog)log.clone();
			noiseLog.removeAll(noiseLog);
			for(XTrace trace : log) {
				if(removeList.contains(log.indexOf(trace))) {
					noiseLog.add(trace);
				}
			}
			return noiseLog;
			
		} else if(filteringType == 2) { // Only Clean
			System.out.println("Clean Log filtering");
			for(XTrace trace : log) {
				String eventList = "";
				int counter = 0;
				String prevTime = "";
